...
bool PolygonZone::isPointOverlappingZone(const types::point_t &position) const {
  return bg::covered_by(position, this->polygon);
}

bool PolygonZone::isBoxOverlappingZone(const types::box_t &box) const {
  return bg::intersects(box, this->polygon);
}
...
types::box_t PolygonZone::getEnvelope() const { return this->envelope; }
