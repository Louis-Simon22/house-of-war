...
public:
  ...
  PolygonZone(const types::box_t &envelope, const Entity *parentEntity);
  PolygonZone(const types::polygon_t &polygon, const Entity *parentEntity);
  ~PolygonZone() override;

public:
  bool isPointOverlappingZone(const types::point_t &position) const override;
  bool isBoxOverlappingZone(const types::box_t &box) const override;
